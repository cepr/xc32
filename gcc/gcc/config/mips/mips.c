...
#include "dumpfile.h"
#include "hwint.h"
#include "mips-machine-function.h"
...
/* PIC32 CP0 access functions */
  ...
  PIC32_BUILTIN_SOFTWARE_BREAKPOINT
...
const enum reg_class mips_regno_to_class[FIRST_PSEUDO_REGISTER] = {
  ...
  COP3_REGS,	COP3_REGS,	COP3_REGS,	COP3_REGS,
  COP3_REGS,	COP3_REGS,	COP3_REGS,	COP3_REGS,
 /* Begin Microchip PIC32 - COP0_REG_SELECT */
  /* 176 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 180 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 184 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 188 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 192 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 196 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 200 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* 204 */ COP0_REGS,  COP0_REGS,  COP0_REGS,  COP0_REGS,
  /* End Microchip PIC32 */
...
static const struct attribute_spec mips_attribute_table[] = {
  ...
  { "longcall",    0, 0, false, true,  true,  NULL, false },
...
#endif /* MIPS_INTERRUPT_ATTRIBUTE */
...
mips_far_type_p (const_tree type)
{
  return (lookup_attribute ("long_call", TYPE_ATTRIBUTES (type)) != NULL
       || lookup_attribute ("longcall", TYPE_ATTRIBUTES (type)) != NULL
...
{
  unsigned int diff = 0;
  unsigned int old_attr =0,new_attr=0;
  
  old_attr = mips_get_compress_on_flags(DECL_ATTRIBUTES (olddecl));
  new_attr = mips_get_compress_on_flags (DECL_ATTRIBUTES (newdecl));

  if(old_attr != 0 && new_attr !=0)
  {
	diff = old_attr ^ new_attr;
  }
  if (diff)
    error ("%qE redeclared with conflicting %qs attributes",
	   DECL_NAME (newdecl), mips_get_compress_on_name (diff));
 
  old_attr = mips_get_compress_off_flags (DECL_ATTRIBUTES (olddecl));
  new_attr = mips_get_compress_off_flags (DECL_ATTRIBUTES (newdecl));
      ...
      if (dest_code == REG)
	{
	  ...
	  if (FP_REG_P (REGNO (dest)))
	    return dbl_p ? "dmtc1\t%z1,%0" : "mtc1\t%z1,%0";
          ...
          /* Added for PIC32 */
          if (COP0_REG_SELECT_P (REGNO (dest)))
            {
              static char retval[] = "dmtc0\t%z1,$__,_";
              if (REGNO(dest) == COP0_SRSCTL_REG_NUM)
                {
                  retval[11] = '1';
                  retval[12] = '2';
                  retval[14] = '2';
...
#ifdef TARGET_MCHP_PIC32MX
          /* Added for PIC32 */
          if (COP0_REG_SELECT_P (REGNO (src)))
            {
              static char retval[] = "dmfc0\t%0,$__,_";
              if (REGNO(src) == COP0_SRSCTL_REG_NUM)
                {
                  retval[10] = '1';
                  retval[11] = '2';
                  retval[13] = '2';
	...
	return false;
	 ...
	 call-clobbered.  */
      return call_really_used_regs[regno];
           ...
           !mchp_function_nofpu_p (TREE_TYPE (current_function_decl)))
    {
       return call_really_used_regs[regno];
    }
#endif
...
mips_compute_frame_info (bool recalculate, struct mips_frame_info *frame)
{
...
#ifdef MIPS_SUBTARGET_COMPUTE_FRAME_INFO
  frame->num_cop0_regs += MIPS_SUBTARGET_COMPUTE_FRAME_INFO();
#endif
  ...
  if (frame->num_fp > 0)
    {
      ...
      /* _BUILD_MCHP_ _BUILD_C32_ */
      /* Add one for FCSR */
      offset += MIPS_STACK_ALIGN (UNITS_PER_WORD);
      frame->fcsr_sp_offset = offset - UNITS_PER_WORD;
    }
 ...
 /* _BUILD_MCHP_ _BUILD_C32_ */
  if (frame->fcsr_sp_offset > 0)
    frame->fcsr_save_offset = frame->fcsr_sp_offset - offset;
...
#if defined(TARGET_MCHP_PIC32MX)
	if (!(BITSET_P (cfun->machine->frame.savedgpr, regno - GP_REG_FIRST)))
          {
            mips_save_restore_reg (word_mode, regno, offset, fn);
            cfun->machine->frame.savedgpr |= (1 << (regno - GP_REG_FIRST));
          }
...
#ifdef MIPS_SUBTARGET_OUTPUT_FUNCTION_PROLOGUE
  MIPS_SUBTARGET_OUTPUT_FUNCTION_PROLOGUE(file,cfun->machine->frame.total_size,size);
#endif /* MIPS_SUBTARGET_OUTPUT_FUNCTION_PROLOGUE */
...
#ifdef MIPS_SUBTARGET_OUTPUT_FUNCTION_EPILOGUE
  MIPS_SUBTARGET_OUTPUT_FUNCTION_EPILOGUE(file,size);
#endif /* MIPS_SUBTARGET_OUTPUT_FUNCTION_EPILOGUE */
...
mips_expand_prologue (void)
{
 ...
 #if defined(MIPS_SUBTARGET_SUPPRESS_PROLOGUE)
  if (MIPS_SUBTARGET_SUPPRESS_PROLOGUE() == true)
    {
      emit_insn (gen_blockage ());
      return;
    }
...
#ifdef MIPS_SUBTARGET_EXPAND_PROLOGUE_SAVEREGS
      size = MIPS_SUBTARGET_EXPAND_PROLOGUE_SAVEREGS(size,step1);
#else
      if (GENERATE_MIPS16E_SAVE_RESTORE)
 	{
 	  HOST_WIDE_INT offset;
 ...
 #endif /* MIPS_SUBTARGET_EXPAND_PROLOGUE_SAVEREGS */
    }
...
#ifdef MIPS_SUBTARGET_EXPAND_EPILOGUE_RESTOREREGS
  MIPS_SUBTARGET_EXPAND_EPILOGUE_RESTOREREGS(step1,step2);
#else
...
#endif /* MIPS_SUBTARGET_EXPAND_EPILOGUE_INTERRUPT */
...
#ifdef TARGET_MCHP_PIC32MX
  /* Interrupt priority 7 can't be interrupted. */
  if (cfun->machine->interrupt_handler_p
      && !cfun->machine->keep_interrupts_masked_p
...
/* Construct a mips_builtin_description from the given arguments.
   ...
   AVAIL is the name of the availability predicate, without the leading
   mips_builtin_avail_.  */
#define _PIC32_BUILTIN(INSN, COND, NAME, BUILTIN_TYPE,                  \
                     FUNCTION_TYPE)                                     \
  { CODE_FOR_pic32_ ## INSN, MIPS_FP_COND_ ## COND,                     \
    "__builtin_" NAME, BUILTIN_TYPE, FUNCTION_TYPE,                     \
    pic32_builtin_avail}
#define PIC32_BUILTIN(INSN, BUILTIN_TYPE, FUNCTION_TYPE)                \
  _PIC32_BUILTIN (INSN, f, #INSN, BUILTIN_TYPE, FUNCTION_TYPE)
#define _PIC32_EXPAND_BUILTIN(COND, NAME, BUILTIN_TYPE,                 \
                     FUNCTION_TYPE)                                     \
  { BUILTIN_TYPE, MIPS_FP_COND_ ## COND,                                \
    "__builtin_" NAME, BUILTIN_TYPE, FUNCTION_TYPE,                     \
    pic32_builtin_avail}
#define PIC32_EXPAND_BUILTIN(INSN, BUILTIN_TYPE, FUNCTION_TYPE)         \
  _PIC32_EXPAND_BUILTIN (f, #INSN, BUILTIN_TYPE, FUNCTION_TYPE)
...
#define CODE_FOR_pic32_unique_id            CODE_FOR_nothing
#define CODE_FOR_pic32_get_isr_state        CODE_FOR_nothing
#define CODE_FOR_pic32_set_isr_state        CODE_FOR_nothing
#define CODE_FOR_pic32_disable_interrupts   CODE_FOR_nothing
#define CODE_FOR_pic32_enable_interrupts    CODE_FOR_nothing
#define CODE_FOR_pic32_software_breakpoint  CODE_FOR_nothing
...
static const struct mips_builtin_description mips_builtins[] = {
  ...
  DIRECT_NO_TARGET_BUILTIN (cache, MIPS_VOID_FTYPE_SI_CVPOINTER, cache),
   /* Microchip Technology PIC32 built-in functions */
  PIC32_BUILTIN (mfc0, PIC32_BUILTIN_MFC0, MIPS_USI_FTYPE_USI_USI),
  PIC32_BUILTIN (mtc0, PIC32_BUILTIN_MTC0, MIPS_VOID_FTYPE_USI_USI_USI),
  PIC32_BUILTIN (mxc0, PIC32_BUILTIN_MXC0, MIPS_USI_FTYPE_USI_USI_USI),
  ...
  PIC32_BUILTIN (unique_id, PIC32_BUILTIN_UNIQUEID, MIPS_USI_FTYPE_CONSTSTRING_SI),
...
#endif
  PIC32_BUILTIN (section_begin, PIC32_BUILTIN_SECTION_BEGIN, MIPS_USI_FTYPE_CONSTSTRING),
...
#define MIPS_ATYPE_CONSTSTRING const_ptr_type_node
...
#ifdef _BUILD_MCHP_
             gcc_assert(ARRAY_SIZE(mips_builtins)==ARRAY_SIZE(mips_builtin_decls));
             if (TARGET_PRINT_BUILTINS)
               mchp_print_builtin_function (mips_builtin_decls[i]);
#endif
    }
#ifdef _BUILD_MCHP_
  /* We cannot call build_function_type_list for this function because we
     need a prototype that looks like unsigned int foo(...) not one that
...
static rtx
pic32_prepare_builtin_arg (enum insn_code icode,
  ...
  if (!insn_data[icode].operand[opno].predicate (value, mode))
    {
      ...
      if (!insn_data[icode].operand[opno].predicate (value, mode))
        {
          error ("invalid argument %d to built-in function", argno + 1);
          return const0_rtx;
        }
    }
...
const char *
pic32_output_switch_ISA32 (void)
{
#define BUFSIZE 131
...
const char *
pic32_output_switch_ISA16 (void)
{
...
static rtx
pic32_prepare_builtin_target (enum insn_code icode, unsigned int op, rtx target)
{
  enum machine_mode mode;
...
static rtx
pic32_expand_mfc0 (enum insn_code icode, rtx target, unsigned int fcode,
                   tree exp)
{
  rtx pat;
  tree argreg, argsel;
  ...
  argreg = CALL_EXPR_ARG (exp, 0);
  argsel = CALL_EXPR_ARG (exp, 1);

  target = pic32_prepare_builtin_target (icode, 0, target);

  /* Do not call mips_prepare_builtin_arg here because it does its own
   * argument type checking. We can generate a more specific error
   * message here. */

  /* Error checking */
  if (TREE_CODE (argreg) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argreg) >
          (unsigned HOST_WIDE_INT) ((1 << 5) - 1))
        {
          warning (0, "argument 1 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 5) - 1);
          opreg = pic32_prepare_builtin_arg (icode, 1, exp, 0);
          opreg = GEN_INT (INTVAL (opreg) & ((1 << 5) - 1));
        }
      else
        {
          opreg =  pic32_prepare_builtin_arg (icode, 1, exp, 0);
  ...
  else
    {
      error ("argument 1 of '%s' must be an immediate integer.",
             mips_builtins[fcode].name);
      opreg =  const0_rtx;
    }

  if (TREE_CODE (argsel) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argsel) >
          (unsigned HOST_WIDE_INT) ((1 << 3) - 1))
        {
          warning (0, "argument 2 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 3) - 1);
          opsel = pic32_prepare_builtin_arg (icode, 2, exp, 1);
          opsel = GEN_INT (INTVAL (opsel) & ((1 << 3) - 1));
        }
      else
        {
          opsel =  pic32_prepare_builtin_arg (icode, 2, exp, 1);
...
#define SWITCH_ISABASE32_SIZE 132
#define SWITCH_ISABASE16_SIZE 122
#define INST_SIZE              30

/* For the __builtin_mfc0(reg,sel) function */
const char *
pic32_output_mfc0_32 (rtx dest, int srcreg, int srcsel)
{
static char retval[SWITCH_ISABASE32_SIZE + INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
  gcc_assert (srcreg >= 0);
  gcc_assert (srcreg < 32);
  gcc_assert (srcsel >= 0);
  gcc_assert (srcsel < 10);

  if (TARGET_MIPS16) {
      snprintf(retval, sizeof(retval)-1, "%s\n\tmfc0\t%s,$%d,%d\n\t%s",
               pic32_output_switch_ISA32(),
               reg_names[REGNO (dest)],
               srcreg, srcsel,
               pic32_output_switch_ISA16()
              );
  } else {
      snprintf(retval, sizeof(retval)-1, "mfc0\t%s,$%d,%d",
               reg_names[REGNO (dest)],
               srcreg, srcsel
               );
  }
...
static rtx
pic32_expand_mtc0 (enum insn_code icode,
                   unsigned int fcode,
                   tree exp)
{

  rtx pat;
  tree argreg, argsel, argval;
  rtx opreg, opsel, opval;
  rtx insns, label;
  gcc_assert (call_expr_nargs (exp) <= insn_data[icode].n_operands);

  argreg = CALL_EXPR_ARG (exp, 0);
  argsel = CALL_EXPR_ARG (exp, 1);
  argval = CALL_EXPR_ARG (exp, 2);
  ...
  opval =  pic32_prepare_builtin_arg (icode, 0, exp, 2);

  emit_insn (gen_blockage ());
  pat = gen_pic32_mtc0 (opval, opreg, opsel);
  emit_insn (pat);
  emit_insn (gen_blockage ());
...
const char *
pic32_output_mtc0_32 ( rtx value, int srcreg, int srcsel)
{
  static char retval[SWITCH_ISABASE32_SIZE + 2*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
  gcc_assert (srcreg >= 0);
  gcc_assert (srcreg < 32);
  gcc_assert (srcsel >= 0);
  gcc_assert (srcsel < 10);

  gcc_assert(0 < REGNO (value));
  gcc_assert(32 > REGNO (value));
  gcc_assert ('$' == reg_names[REGNO (value)][0]);
  
  if (TARGET_MIPS16) {
      snprintf (retval, sizeof(retval)-1, "%s\n\tmtc0\t%s,$%d,%d\n\tehb\n\t%s",
                pic32_output_switch_ISA32(),
                reg_names[REGNO (value)],
                srcreg, 
                srcsel,
                pic32_output_switch_ISA16()
               );
  } else {
      snprintf (retval, sizeof(retval)-1, "mtc0\t%s,$%d,%d\n\tehb",
                reg_names[REGNO (value)],
                srcreg, 
                srcsel
               );
...
static rtx
pic32_expand_mxc0 (enum insn_code icode,
                   rtx target,
                   unsigned int fcode,
                   tree exp)
{
  ...
  if (TREE_CODE (argreg) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argreg) >
          (unsigned HOST_WIDE_INT) ((1 << 5) - 1))
        {
          warning (0, "argument 1 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 5) - 1);
          opreg = pic32_prepare_builtin_arg (icode, 2, exp, 0);
          opreg = GEN_INT (INTVAL (opreg) & ((1 << 5) - 1));
        }
      else
        {
  ...
  if (TREE_CODE (argsel) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argsel) >
          (unsigned HOST_WIDE_INT) ((1 << 3) - 1))
        {
          warning (0, "argument 2 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 3) - 1);
          opsel = pic32_prepare_builtin_arg (icode, 3, exp, 1);
          opsel = GEN_INT (INTVAL (opsel) & ((1 << 3) - 1));
        }
      else
        {
  ...
  opval =  pic32_prepare_builtin_arg (icode, 1, exp, 2);

    if (TARGET_MIPS16)
    {
        emit_insn (gen_blockage ());
        pat = gen_pic32_mxc0 (target, opval, opreg, opsel);
        emit_insn (pat);
        emit_insn (gen_blockage ());
    }
    else
...
/* For the __builtin_mxc0(reg,sel,val) function */
const char *
pic32_output_mxc0_32 ( rtx target, rtx value, int srcreg, int srcsel)
{
  static char retval[SWITCH_ISABASE32_SIZE + 3*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
  gcc_assert (srcreg >= 0);
  gcc_assert (srcreg < 32);
  gcc_assert (srcsel >= 0);
  gcc_assert (srcsel < 10);

  gcc_assert(0 < REGNO (target));
  gcc_assert(32 > REGNO (target));
  gcc_assert(0 < REGNO (value));
  gcc_assert(32 > REGNO (value));

  gcc_assert ('$' == reg_names[REGNO (target)][0]);
  gcc_assert ('$' == reg_names[REGNO (value)][0]);
  
  snprintf (retval, sizeof(retval)-1, "%s\n\tmfc0\t%s,$%d,%d\n\tmtc0\t%s,$%d,%d\n\tehb\n\t%s",
            pic32_output_switch_ISA32(),
            reg_names[REGNO (target)],
            srcreg, 
            srcsel,
            reg_names[REGNO (value)],
            srcreg, 
...
static rtx
pic32_expand_bcc0 (enum insn_code icode,
                   rtx target,
                   unsigned int fcode,
                   tree exp)
{
  ...
  if (TREE_CODE (argsel) == INTEGER_CST)
    ...
    {
        emit_insn (gen_blockage ());
        pat = gen_pic32_bcc0 (target, opval, opreg, opsel);
        emit_insn (pat);
        emit_insn (gen_blockage ());
    }
    else
    {
        ...
        pat = gen_one_cmplsi2(opval, opval);
        if (! pat)
          return 0;
        emit_insn (pat);

        pat = gen_andsi3(opval,target,opval);
        if (! pat)
          return 0;
        emit_insn (pat);
...
/* For the __builtin_bcc0(reg,sel,val) function */

const char *
pic32_output_bcc0_32 ( rtx target, rtx tmp, int regval, int sel)
{
    static char retval[SWITCH_ISABASE32_SIZE + 6*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
    gcc_assert (regval >= 0);
    gcc_assert (regval < 32);

    gcc_assert(0 < REGNO(target));
    gcc_assert(32 > REGNO(target));
    gcc_assert(0 < REGNO(tmp));
    gcc_assert(32 > REGNO(tmp));

    gcc_assert ('$' == reg_names[REGNO (target)][0]);
    gcc_assert ('$' == reg_names[REGNO (tmp)][0]);

    snprintf(retval, sizeof(retval)-1, "%s\n\tmfc0\t%s,$%d,%d\n\tnor\t%s,$0,%s\n\tand\t%s,%s\n\tmtc0\t%s,$%d,%d\n\tehb\n\t%s\n", 
           pic32_output_switch_ISA32(),
           reg_names[REGNO (target)], regval, sel,            //mfc0
           reg_names[REGNO (tmp)], reg_names[REGNO (tmp)],    // nor
           reg_names[REGNO (tmp)], reg_names[REGNO (target)], // and
           reg_names[REGNO (tmp)], regval, sel,               // mtc0
           pic32_output_switch_ISA16()
           );
    return retval;
}
...
static rtx
pic32_expand_bsc0 (enum insn_code icode,
                   rtx target,
                   unsigned int fcode,
                   tree exp)
{
  ...
  if (TREE_CODE (argsel) == INTEGER_CST)
    ...
    {
      emit_insn (gen_blockage ());
      pat = gen_pic32_bsc0 (target, opval, opreg, opsel);
      emit_insn (pat);
      emit_insn (gen_blockage ());
    }
    else
    {
      ...
      pat = gen_iorsi3(opval,target,opval);
      if (! pat)
       	return 0;
      emit_insn (pat);
...
const char *
pic32_output_bsc0_32 ( rtx target, rtx tmp, int regval, int sel)
{
    static char retval[SWITCH_ISABASE32_SIZE + 4*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
    ...
    snprintf(retval, sizeof(retval)-1, "%s\n\tmfc0\t%s,$%d,%d\n\tor\t%s,%s\n\tmtc0\t%s,$%d,%d\n\tehb\n\t%s\n", 
           pic32_output_switch_ISA32(),
           reg_names[REGNO (target)], regval, sel,            // mfc0
           reg_names[REGNO (tmp)], reg_names[REGNO (target)], // or
           reg_names[REGNO (tmp)], regval, sel,               // mtc0
           pic32_output_switch_ISA16()
           );
...
/* Expand BCSC0 */
/* mips_expand_bcsc0 for __builtin_bcsc0(reg,sel,clear,set) */
static rtx
pic32_expand_bcsc0 (enum insn_code icode,
                    rtx target,
                    unsigned int fcode,
                    tree exp)
{
  rtx pat;
  tree argreg, argsel, argclr, argset;
  rtx opreg, opsel, opclr, opset;

  argreg = CALL_EXPR_ARG (exp, 0);
  argsel = CALL_EXPR_ARG (exp, 1);
  argclr = CALL_EXPR_ARG (exp, 2);
  argset = CALL_EXPR_ARG (exp, 3);
  ...
  if (TREE_CODE (argreg) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argreg) >
          (unsigned HOST_WIDE_INT) ((1 << 5) - 1))
        {
          warning (0, "argument 1 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 5) - 1);
          opreg = pic32_prepare_builtin_arg (icode, 3, exp, 0);
          opreg = GEN_INT (INTVAL (opreg) & ((1 << 5) - 1));
        }
      else
        {
  ...
  if (TREE_CODE (argsel) == INTEGER_CST)
    {
      if (TREE_INT_CST_LOW (argsel) >
          (unsigned HOST_WIDE_INT) ((1 << 3) - 1))
        {
          warning (0, "argument 2 of `%s' not in the range 0 to %d.",
                   mips_builtins[fcode].name, (1 << 3) - 1);
          opsel = pic32_prepare_builtin_arg (icode, 4, exp, 1);
          opsel = GEN_INT (INTVAL (opsel) & ((1 << 3) - 1));
        }
      else
        {
  ...
  opclr =  pic32_prepare_builtin_arg (icode, 0, exp, 2);
  opset =  pic32_prepare_builtin_arg (icode, 1, exp, 3);

    if (TARGET_MIPS16)
    {
        emit_insn (gen_blockage ());
        pat = gen_pic32_bcsc0(target,opclr,opset,opreg,opsel);
        emit_insn (pat);
        emit_insn (gen_blockage ());
    }
    else
    {
        ...
        pat = gen_one_cmplsi2(opclr, opclr);
        if (! pat)
          return 0;
        emit_insn (pat);

        pat = gen_andsi3(opclr,target,opclr);
        if (! pat)
          return 0;
        emit_insn (pat);

        pat = gen_iorsi3(opclr,opclr,opset);
        if (! pat)
          return 0;
        emit_insn (pat);

        pat = gen_pic32_mtc0 (opclr, opreg, opsel);
        if (! pat)
          return 0;
        emit_insn (pat);
    }
...
const char * 
pic32_output_bcsc0_32 (rtx target, int opclr, int opset, rtx opreg, rtx opsel)
{
  static char retval[SWITCH_ISABASE32_SIZE + 6*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
  ...
  gcc_assert (32 >  REGNO(target));
    ...
    snprintf(retval,sizeof(retval)-1,
             "%s\n\tmfc0\t%s,$%d,%d\n\tnor\t$%d,$0,$%d\n\tand\t$%d,%s\n\tor\t$%d,$%d\n\tmtc0\t$%d,$%d,%d\n\tehb\n\t%s",
             pic32_output_switch_ISA32(),
             reg_names[REGNO (target)],
             REGNO (opreg),
...
#undef SWITCH_ISABASE32_SIZE
#undef SWITCH_ISABASE16_SIZE
#undef INST_SIZE     
  ...
  if (TREE_CODE(sub_arg0) == ADDR_EXPR)
    {
      ...
      if (TREE_CODE(sub_arg0) == STRING_CST)
        {
          section_name = (char *)TREE_STRING_POINTER(sub_arg0);
        }
     }
  if (section_name == 0)
    {
                   ...
                   tree exp)
{
  ...
  target = pic32_prepare_builtin_target (CODE_FOR_pic32_mfc0, 0, target);
  emit_insn (gen_blockage ());

    if (TARGET_MIPS16)
    {
        pat = gen_get_isr_state_mips16(target, temp);
        if (! pat)
            return 0;
        emit_insn (pat);
    }
...
const char * 
pic32_output_get_isr_state_mips16 (rtx target, rtx srcreg)
{
    static char retval[SWITCH_ISABASE32_SIZE + 4*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};
    ...
    gcc_assert (0  <  REGNO(srcreg));
    gcc_assert (32 >  REGNO(srcreg));

    snprintf(retval, sizeof(retval)-1, "%s\n\tmfc0\t%s,$12,0\n\tsrl\t%s,%s,10\n\tins\t%s,%s,3,1\n\tandi\t%s,%s,0x7\n\t%s\n",
           pic32_output_switch_ISA32(),
           reg_names[REGNO (srcreg)],            //mfc0
           reg_names[REGNO (target)], reg_names[REGNO (srcreg)] ,// srl
           reg_names[REGNO (target)], reg_names[REGNO (srcreg)] ,// ins
           reg_names[REGNO (target)], reg_names[REGNO (target)] ,// andi
           pic32_output_switch_ISA16()
           );
    return retval;
}
                   ...
                   tree exp)
{
  ...
  opval =  pic32_prepare_builtin_arg (CODE_FOR_pic32_mtc0, 0, exp, 0);
 
  emit_insn (gen_blockage ());

    if (TARGET_MIPS16)
    {
        pat = gen_set_isr_state_mips16(save_sr, temp, opval);
        if (! pat)
            return 0;
        emit_insn (pat);
    }
...
const char *	
pic32_output_set_isr_state_mips16 (rtx tmpreg, rtx srcreg, rtx value)
{
    static char retval[SWITCH_ISABASE32_SIZE + 7*INST_SIZE + SWITCH_ISABASE16_SIZE] = {0};

    gcc_assert (0  <  REGNO(tmpreg));
    gcc_assert (32 >  REGNO(tmpreg));
    ...
    snprintf(retval, sizeof(retval)-1, "%s\n\tdi\n\tehb\n\tmfc0\t%s,$12,0\n\tins\t%s,%s,10,3\n\tsrl\t%s,%s,3\n\tins\t%s,%s,0,1\n\tmtc0\t%s,$12,0\n\tehb\n\t%s\n",
           pic32_output_switch_ISA32(),
           reg_names[REGNO (tmpreg)],                               // mfc0
           reg_names[REGNO (tmpreg)], reg_names[REGNO (value)] ,   // ins
           reg_names[REGNO (srcreg)], reg_names[REGNO (value)] ,   // srl
           reg_names[REGNO (tmpreg)], reg_names[REGNO (srcreg)] ,   // ins
           reg_names[REGNO (tmpreg)],                               // mtc0
           pic32_output_switch_ISA16()
           );
...
pic32_output_ei ( rtx value)
{
  ...
  if (TARGET_MIPS16) {
      ...
      snprintf (retval, sizeof(retval)-1, "ei\t%s",
                reg_names[REGNO (value)]
               );
  }
  return retval;
...
pic32_output_di ( rtx value)
{
  ...
  if (TARGET_MIPS16) {
      ...
      snprintf (retval, sizeof(retval)-1, "di\t%s\n\tehb",
                reg_names[REGNO (value)]
               );
  }
  return retval;
                   ...
                   tree exp)
{
  ...
  target = pic32_prepare_builtin_target (CODE_FOR_pic32_di, 0, target);
  emit_insn (gen_blockage ());
                   ...
                   tree exp)
{
  ...
  target = pic32_prepare_builtin_target (CODE_FOR_pic32_ei, 0, target);
  emit_insn (gen_blockage ());
 ...
 * promoted to a double. This might not be a problem because the code
 * that is using __builtin_ittype is also using a vararg function. */
static rtx
pic32_expand_ittype (tree exp)
{
  tree arg0;
  int result = 0;
  tree type;
		     ...
		     enum machine_mode mode, int ignore)
{
...
#if defined(TARGET_MCHP_PIC32MX)
  /* Builtin functions that do not go through the standard mips_builtins
   * initialization. */
  switch (fcode)
    {
      /* ittype */
    case PIC32_BUILTIN_ITTYPE:
      return pic32_expand_ittype (exp);
  ...
  switch (d->builtin_type)
    {
    /* NOP */
    case PIC32_BUILTIN_NOP:
      pic32_expand_nop ();
      return NULL_RTX;

      /* MFC0 */
    case PIC32_BUILTIN_MFC0:
      return pic32_expand_mfc0 (d->icode, target, fcode, exp);

      /* MTC0 */
    case PIC32_BUILTIN_MTC0:
        return pic32_expand_mtc0 (d->icode, fcode, exp);

      /* MXC0 */
    case PIC32_BUILTIN_MXC0:
      return pic32_expand_mxc0 (d->icode, target, fcode, exp);

      /* BCC0 */
    case PIC32_BUILTIN_BCC0:
      return pic32_expand_bcc0 (d->icode, target, fcode, exp);

      /* BSC0 */
    case PIC32_BUILTIN_BSC0:
      return pic32_expand_bsc0 (d->icode, target, fcode, exp);

      /* BCSC0 */
    case PIC32_BUILTIN_BCSC0:
      return pic32_expand_bcsc0 (d->icode, target, fcode, exp);
#if defined(TARGET_MCHP_PIC32MX)
      /* unique_id */
    ...
    case PIC32_BUILTIN_SOFTWARE_BREAKPOINT:
      return pic32_expand_software_breakpoint_builtin ();
#endif
    default:
      break;
    }
...
{
  return ((struct machine_function *)(ggc_internal_cleared_alloc_stat (sizeof (struct machine_function) MEM_STAT_INFO)));
}
...
/* Implement TARGET_FN_OTHER_HARD_REG_USAGE.  For TARGET_SPLIT_CALLS,
   register $6 is actually clobbered inside the GP-reload created by the
   post-reload split pass.  Express that here for sake correctness of
   -fuse-caller-saves.  */
static void
